...
const NowPlaying = () => {
  ...
  const queueIdMap = useRef(new WeakMap());
  ...
  const handleRefreshQueue = () => {
    ...
    setTimeout(() => setIsRefreshingQueue(false), 1000);
  };
  ...
  }, [isNowPlayingOpen, setIsQueueVisible]);
      ...
      <React.Fragment key={i}>
        <span 
          onClick={async (e) => { 
            ...
            // Re-use logic to find artist ID and route
            try {
              const res = await fetch(`${API_URL}/search?q=${encodeURIComponent(artist)}&filter=artists`);
              if (res.ok) {
                const data = await res.json();
                if (data && data.length > 0 && data[0].youtube_id) {
                  router.push(`/artist/${data[0].youtube_id}`);
                }
              }
            } catch (err) {
        ...
        >
                ...
                <div className="flex flex-col flex-1 min-w-0 pr-4 text-left">
                  <h1 
                    ...
                    onClick={async (e) => {
                      ...
                      const albumName = typeof currentTrack.album === 'object' ? currentTrack.album?.name : currentTrack.album;
                      const artistName = currentTrack.artist || currentTrack.artist_name || '';
                      
                      const searchStr = (!albumName || albumName === 'Single') 
                        ? currentTrack.title + ' ' + artistName 
                        : albumName + ' ' + artistName;

                      try {
                        const res = await fetch(`${API_URL}/search?q=${encodeURIComponent(searchStr)}&filter=albums`);
                        if (res.ok) {
                          const data = await res.json();
                          if (data && data.length > 0) {
                            let validTarget = null;
                            for (const target of data.slice(0, 3)) { // Check top 3 results
                              const type = target.type === 'playlist' ? 'playlist' : 'album';
                              const id = target.youtube_id || target.id;
                              try {
                                const checkRes = await fetch(`${API_URL}/${type}/${id}`);
                            ...
                            if (validTarget) {
                              router.push(`/${validTarget.type}/${validTarget.id}?highlight=${currentTrack.youtube_id}`);
                              setIsNowPlayingOpen(false);
                              setIsQueueVisible(false);
                            }
                          }
          ...
          <div className={`w-[36.8421%] h-full flex flex-col justify-center pl-10 pr-6 transition-opacity duration-700 ${isQueueVisible ? 'opacity-100 delay-300' : 'opacity-0 pointer-events-none'}`}>
             <div className="w-full h-[82vh] max-h-[760px] bg-white/5 backdrop-blur-3xl border border-white/10 rounded-3xl p-6 flex flex-col overflow-hidden shadow-2xl relative">
                <div className="flex items-center justify-between mb-6 relative">
                   <div className="flex items-center gap-3 shrink-0">
                     ...
                     <button 
                       ...
                       className="p-1.5 rounded-full text-zinc-400 hover:text-white hover:bg-white/10 transition-all hover:scale-110"
                   ...
                   <div ref={searchContainerRef} className={`relative z-40 group flex flex-col transition-all duration-500 ease-out ${isSearchFocused ? 'w-full ml-4' : 'w-72 ml-4'}`}>
                     ...
                     <div className="relative w-full rounded-full overflow-hidden shadow-[0_10px_30px_-10px_rgba(0,0,0,0.5)] bg-zinc-800 p-[1.5px]">
                       <LiquidMetal colorBack="#555555" colorTint="#ffffff" speed={0.4} repetition={4} distortion={0.15} scale={1} className="absolute inset-0 z-0 rounded-full" />
                       
                       <div className="relative z-10 rounded-full flex items-center bg-black transition-colors duration-300 focus-within:bg-zinc-900/90 w-full pl-4 pr-1.5 py-1 shadow-[inset_0_2px_4px_rgba(0,0,0,0.5)]">
                         <input
                           type="text"
                           ...
                           className="flex-1 bg-transparent border-none text-white placeholder:text-zinc-500 text-xs outline-none transition-all duration-500 py-1 min-w-0"
                         />
                ...
                <div className={`flex-1 overflow-y-auto no-scrollbar flex flex-col pr-2 pb-10 transition-all duration-500 ${isSearchFocused && (query.trim().length > 0 || isSearching) ? 'blur-[8px] opacity-40 scale-[0.98] pointer-events-none' : 'blur-0 opacity-100 scale-100'}`}>
                     ...
                     >
                       <AnimatePresence initial={false}>
                         {queue.slice(currentIndex + 1).map((track, idx) => {
                            let uniqueKey = queueIdMap.current.get(track);
                            if (!uniqueKey) {
                               uniqueKey = `drag-${Date.now()}-${Math.random()}`;
                               queueIdMap.current.set(track, uniqueKey);
                            }
                            return (
                              <Reorder.Item 
                                value={track} 
                                key={queueIdMap.current.get(track) || track.id || track.youtube_id}
                                initial={{ opacity: 0, height: 0, scale: 0.9 }}
                                animate={{ opacity: 1, height: 'auto', scale: 1 }}
                                exit={{ opacity: 0, height: 0, scale: 0.9, transition: { duration: 0.3 } }}
