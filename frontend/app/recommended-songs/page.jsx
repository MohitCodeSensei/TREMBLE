...
const RecommendedSongs = () => {
  const { setCurrentTrack, setIsNowPlayingOpen, setIsPlaying, user } = usePlayer();
  ...
  useEffect(() => {
    const fetchRecommended = async () => {
      try {
        ...
        const res = await fetch(`${API_URL}/api/recommended?limit=25${userId ? `&user_id=${userId}` : ''}`);
      ...
      <div className="flex items-center gap-8 border-b border-zinc-800 pb-4 overflow-x-auto">
        {tabs.map((tab) => (
          <Link
            key={tab.name}
            href={tab.path}
            className={`text-lg font-bold drop-shadow-lg transition-all whitespace-nowrap ${tab.name === 'Recommended Songs' ? 'text-white border-b-2 border-white' : 'text-zinc-500 hover:text-zinc-300'}`}
          >
            {tab.name}
          </Link>
        ))}
