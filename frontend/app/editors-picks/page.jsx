...
const EditorsPicks = () => {
  ...
  useEffect(() => {
    const fetchEditorsPicks = async () => {
      try {
        const res = await fetch(`${API_URL}/api/editors-picks?limit=25`);
        const data = await res.json();
        setSongs(data);
      } catch (err) {
        console.error("Failed to fetch editors picks songs", err);
      } finally {
        setIsLoading(false);
      }
    };
      ...
      <div className="flex items-center gap-8 border-b border-zinc-800 pb-4 overflow-x-auto">
        {tabs.map((tab) => (
          <Link
            key={tab.name}
            href={tab.path}
            className={`text-lg font-bold drop-shadow-lg transition-all whitespace-nowrap ${tab.name === 'Editor\'s Picks' ? 'text-white border-b-2 border-white' : 'text-zinc-500 hover:text-zinc-300'}`}
          >
            {tab.name}
          </Link>
        ))}
