...
const ArtistPage = () => {
  ...
  const scrollSimilar = (direction) => {
    if (similarRef.current) {
      const scrollAmount = 552; // 3 items (40rem = 160px) + 3 gaps (1.5rem = 24px) = 184 * 3
  ...
  useEffect(() => {
    const fetchArtist = async () => {
      try {
        setLoading(true);
        const res = await fetch(`${API_URL}/artist/${encodeURIComponent(id)}`);
        if (!res.ok) throw new Error('Failed to fetch artist details');
        const data = await res.json();
        if (data.error) throw new Error(data.error);
        if (data.name) {
          const cleanSlug = encodeURIComponent(String(data.name).trim().toLowerCase().replace(/\s+/g, '+')).replace(/%2B/gi, '+');
          router.replace(`/trembler/${cleanSlug}`);
          return;
        }
        setArtist(data);
      } catch (err) {
  ...
  const handlePlaySong = (song, idx) => {
    ...
    // Queue up the rest of the top songs starting from this one
      ...
      <div className="relative w-full max-w-7xl mx-auto px-8 pt-32 pb-8 flex items-end">
         ...
         <div className="relative z-10 w-full flex flex-col md:flex-row items-center md:items-end gap-8">
            ...
            <div className="flex flex-col flex-1 text-center md:text-left mb-2">
               <h1 className="text-5xl md:text-7xl font-black text-white tracking-tighter mb-4 drop-shadow-[0_4px_12px_rgba(0,0,0,0.5)]">{artist.name}</h1>
               <p className="text-white/80 text-sm md:text-base leading-relaxed line-clamp-3 font-medium max-w-3xl drop-shadow-[0_2px_8px_rgba(0,0,0,0.8)]">
                 {artist.description || `Explore the top hits and popular tracks from ${artist.name}.`}
               </p>
            </div>
         </div>
      </div>
              ...
              <div className="w-full max-w-[1080px]">
                ...
                >
                   {artist.similar_artists.map((sim, idx) => (
                      <div key={idx} className="flex flex-col items-center gap-4 group cursor-pointer w-40 flex-shrink-0 snap-start" onClick={() => router.push(getTremblerUrl(sim.name || sim.id))}>
